...
public class FundOverviewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final String LOG_TAG = FundOverviewAdapter.class.getSimpleName();
    final private Context mContext;
    private Cursor mCursor;
    private TableLayout mChartTable;

    public FundOverviewAdapter(Context context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View item;
        switch (viewType){
            // If it is the first view, return viewholder for FundPortfolio overview
            case 0:
                item = LayoutInflater.from(mContext).inflate(R.layout.adapter_fund_overview, parent, false);
                return new FundOverviewViewHolder(item);
            case 1:
                item = LayoutInflater.from(mContext).inflate(R.layout.adapter_piechart, parent, false);
                return new FundPieChartViewHolder(item);
            default:
                item = LayoutInflater.from(mContext).inflate(R.layout.adapter_table, parent, false);
                return new FundChartViewHolder(item);
        }
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        switch (holder.getItemViewType()) {
            case 0:
                FundOverviewViewHolder viewHolder = (FundOverviewViewHolder) holder;
                mCursor.moveToPosition(position);

                double totalGain = mCursor.getDouble(mCursor.getColumnIndex
                        (PortfolioContract.FundPortfolio.COLUMN_TOTAL_GAIN));
                ...
                double buyTotal =  mCursor.getDouble(
                        mCursor.getColumnIndex(PortfolioContract.FundPortfolio.COLUMN_BUY_TOTAL));
                ...
                viewHolder.soldTotal.setText(String.format(formatter.format(mCursor.getDouble(
                        mCursor.getColumnIndex(PortfolioContract.FundPortfolio.COLUMN_SOLD_TOTAL)))));
                viewHolder.currentTotal.setText(String.format(formatter.format(mCursor.getDouble(
                        mCursor.getColumnIndex(PortfolioContract.FundPortfolio.COLUMN_CURRENT_TOTAL)))));
                ...
                break;
            case 1:
                FundOverviewAdapter.FundPieChartViewHolder piechartHolder = (FundOverviewAdapter.FundPieChartViewHolder) holder;
                    ...
                    do{
                        ...
                        if (currentPercent > 0) {
                            if (dataCursor.getPosition() < 8) {
                                String substring = "";
                                if(symbol.length() > 17){
                                    substring = symbol.substring(0,16);
                                } else {
                                    substring = symbol;
                                }
                                entries.add(new PieEntry(currentPercent, substring));
                            } else {
            ...
            default:
                ...
                final FundChartViewHolder chartHolder = (FundChartViewHolder) holder;
    ...
    class FundOverviewViewHolder extends RecyclerView.ViewHolder {
        ...
        public FundOverviewViewHolder(View itemView) {
            super(itemView);
            ButterKnife.bind(this, itemView);
        }
    }

    class FundPieChartViewHolder extends RecyclerView.ViewHolder implements OnChartValueSelectedListener {
        ...
        public FundPieChartViewHolder(View itemView) {
            super(itemView);
            ButterKnife.bind(this, itemView);
        }
    ...
    class FundChartViewHolder extends RecyclerView.ViewHolder {

        @BindView(R.id.tableChart)
        TableLayout tableChart;

        public FundChartViewHolder(View itemView) {
            super(itemView);
            ButterKnife.bind(this, itemView);
        }
    }
    ...
    private Cursor getDataCursor(){
        String[] affectedColumn = {PortfolioContract.FundData.COLUMN_CURRENT_PERCENT,
                PortfolioContract.FundData.COLUMN_SYMBOL};
        String sortOrder = PortfolioContract.FundData.COLUMN_CURRENT_PERCENT + " DESC";
    ...
    private Cursor getYears(){
        ...
        String[] selectionArguments = {String.valueOf(Constants.ProductType.FUND)};
    ...
    private Cursor getBuyGrowthCursor(String year){
        ...
        String[] selectionArguments = {String.valueOf(Constants.ProductType.FUND), year};
    ...
    private double getBuyGain(String currentMonth, String lookaheadMonth){
        ...
        // Fund Income
        String selection = PortfolioContract.FundTransaction.COLUMN_TIMESTAMP + " >= ? AND " + PortfolioContract.FundTransaction.COLUMN_TIMESTAMP + " < ? AND "
